import axios from 'axios';

const API = '//pokeapi.co/api/v2';
const CACHE_MS = 24 * 60 * 60 * 1000; // one day
const MAX_REQUESTS = 8;
const queue = [];
let nRequests = 0;

const localStorage = window.localStorage;

const get = function(url) {
  let record;
  const promise = new Promise((resolve, reject) => {
    record = { url, resolve, reject };
  });
  queue.push(record);
  fetchNext();
  return promise;
};

const fetchNext = async function() {
  if (nRequests >= MAX_REQUESTS || queue.length === 0) {
    return;
  }
  const record = queue.shift();
  const { url, resolve, reject } = record;
  let cached = null;
  try {
    cached = JSON.parse(localStorage[url]);
  } catch (e) {
    // ignore
  }
  if (cached && cached.timestamp && cached.timestamp >= Date.now() - CACHE_MS) {
    console.debug(`Retrieved from cache: ${url}`);
    resolve(cached.data);
    setTimeout(fetchNext, 0);
  } else {
    console.debug(`Fetching via AJAX: ${url}`);
    nRequests++;
    try {
      const response = await axios.get(url);
      console.debug(`Fetched from API, caching: ${url}`);
      const data = response.data;
      setTimeout(() => {
        const timestamp = Date.now();
        localStorage[url] = JSON.stringify({ data, timestamp });
      }, 0);
      resolve(data);
    } catch (err) {
      reject(err);
    } finally {
      nRequests--;
      setTimeout(fetchNext, 0);
    }
  }
};

export default {
  queue,
  getPokemonByName: async function(name) {
    return await get(`${API}/pokemon/${name}`);
  },
  getMoveByName: async function(name) {
    return await get(`${API}/move/${name}`);
  },
  getPokemonsList: async function() {
    return await get(`${API}/pokemon/?limit=100000&offset=0`);
  },
};
